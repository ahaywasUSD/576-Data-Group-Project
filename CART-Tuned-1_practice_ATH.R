################################################################################
##This is the standard Classification Tree Model & manually tuned parameters. ##
################################################################################
...
class_spec <- decision_tree(min_n = 20, #minimum number of observations for split
                            ...
                            cost_complexity = 0.00025) %>% #regularization parameter
  set_engine("rpart") %>%
  set_mode("classification")
print(class_spec)

#ESTIMATING THE MODEL (CAN BE DONE IN ONE STEP ABOVE WITH EXTRA %>%)
class_fmla <- Risk.Profile ~ .
...
confusion <- table(pred_class_in$.pred_class, pred_class_in$Risk.Profile)
...
confusion <- table(pred_class_out$.pred_class, pred_class_out$Risk.Profile)
confusionMatrix(confusion) #FROM CARET PACKAGE
